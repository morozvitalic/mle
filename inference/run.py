...
def main():
    ...
    target = infer_data[['target']]
    infer_data = infer_data.drop(columns=['target'])

    results = predict_results(model, infer_data)
    store_results(results, args.out_path)

    logging.info(f'Real results: \n{target}')
    logging.info(f'Prediction results: \n{results}')

    res = results['results']
    target = target['target']

    logging.info(f'f1_score: {f1_score(target, res, average="weighted")}')
    logging.info(f'accuracy_score: {accuracy_score(target, res)}')
